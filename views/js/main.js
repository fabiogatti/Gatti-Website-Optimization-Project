...
var resizePizzas = function(size) {
  ...
  // First change made: Using 'getElemenByClassName' function to get the array and only call function determineDX 1 time, to calculate the ned width
  // then make a loop to change all the 'randomPizzaContainer' elements
  // Iterates through pizza elements on the page and changes their widths
  function changePizzaSizes(size) {
    //Previous width calculator
    /*query = document.querySelectorAll(".randomPizzaContainer");
    for (var i = 0; i < query.length; i++) {
      var dx = determineDx(query[i], size);
      var newwidth = (query[i].offsetWidth + dx) + 'px';
      query[i].style.width = newwidth;
    }*/
    //New width calculator
...
//Second change made: Using the function 'getElementsByClassName' we get all the movers, then we calculate (only once) the values for the sin function
//and assign them to 5 variables, which then will be used to cover the 5 posibilities for the position of the background pizzas.
function updatePositions() {
  frame++;
  window.performance.mark("mark_start_frame");


  //Previous mover for pizzas
  ...
  //New mover for pizzas
  var items = document.getElementsByClassName("mover");
  var phase1 = Math.sin((document.body.scrollTop / 1250) + 1)*100;
  var phase2 = Math.sin((document.body.scrollTop / 1250) + 2)*100;
  var phase3 = Math.sin((document.body.scrollTop / 1250) + 3)*100;
  var phase4 = Math.sin((document.body.scrollTop / 1250) + 4)*100;
  var phase5 = Math.sin((document.body.scrollTop / 1250) + 0)*100;
  var phase;
  for (var i = 0; i < items.length; i++) {
    if(i%5==1){
      phase=phase1;
